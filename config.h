...
static const char col_gray4[]       = "#61afef";
...
static const char *autostart[] = {
    "/home/green/.dwm/autostart.sh", NULL
};
...
/* the bottom void of config.h */
static void
autostart_exec(void)
{
	for (size_t i = 0; autostart[i]; i++) {
		if (fork() == 0) {
			setsid();
			execl("/bin/sh", "sh", "-c", autostart[i], (char *)NULL);
			fprintf(stderr, "dwm: execvp %s", autostart[i]);
			perror(" failed");
			_exit(EXIT_SUCCESS);
		}
	}
